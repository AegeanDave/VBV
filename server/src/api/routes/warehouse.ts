...
import { sendVerifiySMS, sendRegistrationSMS } from '../../provider/twilio'
...
export default (app: Router) => {
	...
	route.post('/getVerificationCode', async (req: Request, res: Response) => {
		...
		try {
			const retreiveWarehouseResult = await query(queryName.getWarehouseId, [
				phone.tel,
				phone.countryCode
			])
			if (
				retreiveWarehouseResult.data &&
				retreiveWarehouseResult.data.length > 0
			) {
				...
				if (!smsResult.errorCode) {
					...
					const entirePhoneNumber =
						countryCodes[phone.countryCode].value + phone.tel
					myCache.set(
						entirePhoneNumber,
						{
							verificationCode,
							...retreiveWarehouseResult.data[0]
						},
						1800
					)
				} else {
					res.send({
						status: Status.FAIL,
						message: smsResult.errorMessage
					})
				}
			} else {
				res.send({
	...
	route.post('/login', async (req: Request, res: Response) => {
		const { entirePhoneNumber, verificationCode } = req.body
		const loginInfo: any = myCache.get(entirePhoneNumber)
		if (!loginInfo) {
			res.status(401).send({
				status: Status.FAIL,
			...
			if (loginInfo.verificationCode === verificationCode) {
				...
				const value: Session = {
					...
					warehouseId: loginInfo.warehouseId
				}
				myCache.set(sessionKey, value, 2592000)
				res.status(200).send({ sessionKey, ...loginInfo })
	...
	route.post(
		...
		async (req: Request, res: Response) => {
			...
			const verificationCode = makeVerificationCode()
			try {
				const result = await sendVerifiySMS(phoneNumber, verificationCode)
				if (!result.errorCode) {
					res.send({
						status: Status.SUCCESS
					})
